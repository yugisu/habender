...
const Month = ({monthObj, todos}) => {
  ...
  //#region 
  const calendarDateObjects = [...Array(6).fill([...Array(7).keys()].map(elem => elem + 1))]
    .map((week, idxW) => 
      week.map((day, idxD) => {
    ...
    <div className='month'>
      {calendarDateObjects.map((week, idxW) => 
        week.map((day, idxD) => {
          ...
          const dayInTodos = todos.filter((el) => !isFromOtherMonth && el.date.date === day.date)[0];
          let activities = [];
          if (dayInTodos) { activities = dayInTodos.activities; }
          return <CalendarTile dayObj={day} 
